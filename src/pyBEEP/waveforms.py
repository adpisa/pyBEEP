...
def constant_waveform(pid: bool, value: float, duration: float) -> np.ndarray:
    point_interval = POINT_INTERVAL_POT if pid else POINT_INTERVAL_GAL
    length = int(duration / point_interval)
    return np.full(length, value, dtype=np.float32)

def linear_sweep(pid: bool, start: float, end: float, scan_rate: float) -> np.ndarray:
    point_interval = POINT_INTERVAL_POT if pid else POINT_INTERVAL_GAL
